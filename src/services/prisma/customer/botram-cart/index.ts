...
const getBotramCart = async (req: Request):
...
const addMenuToBotramCart = async (req: Request):
...
const getItemsByBotramGroup = async (req: Request): 
...
const updateQtyOfBotramCartItem = async (req: Request):
...
const deleteBotramCartItem = async (req: Request):
  Promise<BotramGroupMenuCart['id'] | BotramGroupCustomMenuCart['id'] | Error> => {
    const { id: customerId } = req.user as Pick<Customer, 'id' | 'email'>;
    const { botramCartId } = req.params;
    if (!botramCartId) {
